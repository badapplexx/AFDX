...
void NetworkStatistics::createRecorder(RecordType_t type, SwitchDefinition sw, int index)
{
    ...
    switch (type) {
        ...
        case SWITCH_QUEUE_LENGTH_PER_VL_PER_SW: {
            //index = ES index
...
void NetworkStatistics::createQueueLengthCounter(SwitchDefinition s)
{
    if ((-1 != s.index) && s.isA)
        this->queueCounterPerSwitch[s.index] = 0;
}
int NetworkStatistics::getQueueLengthCount(SwitchDefinition s)
{
    int ret = -1;
    if ((-1 != s.index) && s.isA) {
...
void NetworkStatistics::increaseQueueLengthCount(SwitchDefinition s)
{
    if ((-1 != s.index) && s.isA) {
        this->queueCounterPerSwitch[s.index]++;
    }
}
void NetworkStatistics::decreaseQueueLengthCount(SwitchDefinition s)
{
    if ((-1 != s.index) && s.isA) {
        this->queueCounterPerSwitch[s.index]--;
    }
...
int NetworkStatistics::record(RecordType_t type, VLID_t vlid, double value2Record, int swIndex)
{
    int ret = 0;
    RecordPerVL_t *recorPerVl = NULL;
    RecordPerVLPerIndex_t *recordPerVLPerIndex = NULL;
