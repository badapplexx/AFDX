...
void NetworkStatistics::createRecorder(RecordType_t type, VLID_t vlid,
        int index) {
    ...
    case CREDIT_PER_VL_PER_SW: {
        ...
        name = "TokenBucketCredit#SW" + std::to_string(index) + "_VL"
                + toHexString(vlid);
        break;
    }
    case SWITCH_QUEUEING_TIME_PER_VL_PER_SW: {
        ...
        name = "SWQueueingTime#SW" + std::to_string(index) + "_VL"
                + toHexString(vlid);
        break;
    }
    case E2E_LATENCY_PER_VL_PER_RECEIVER_ES: {
        recordPerVLPerIndex = &e2eLatencyAtRxES;
        name = "LatencyAt#ES" + std::to_string(index) + "_VL"
                + toHexString(vlid);
        break;
    }
    default: {
...
void NetworkStatistics::createRecorder(RecordType_t type, SwitchDefinition sw,
        int index) {
    ...
    case SWITCH_QUEUE_LENGTH_PER_SW_PER_PORT: {
        ...
        name = "SWQueueLength#Port" + std::to_string(index) + "_SW"
    ...
    case SWITCH_QUEUEING_TIME_PER_SW_PER_PORT: {
        ...
        name = "SWQueueingTime#Port" + std::to_string(index) + "_SW"
                + std::to_string(sw.index);
        break;
    }
...
void NetworkStatistics::increaseQueueLengthCountInBit(SwitchDefinition s,
...
void NetworkStatistics::decreaseQueueLengthCountInBit(SwitchDefinition s,
        uint64_t messageLenInBit) {
    if ((-1 != s.index) && s.isA) {
        this->queueCounterPerSwitch[s.index]--;
        this->queueCounterInBitPerSwitch[s.index] -= messageLenInBit;
...
int NetworkStatistics::record(RecordType_t type, VLID_t vlid,
        double value2Record, int swIndex) {
    int ret = 0;
    RecordPerVL_t *recorPerVl = NULL;
    RecordPerVLPerIndex_t *recordPerVLPerIndex = NULL;
...
void NetworkStatistics::record(RecordType_t type, SwitchDefinition sw,
        double value2Record, int index) {
    RecordPerSW_t *recordPerSW = NULL;
    RecordPerSWPerIndex_t *recordPerSWPerIndex = NULL;
    switch (type) {
    case SWITCH_QUEUE_LENGTH_PER_SWITCH: {
...
NetworkStatistics::SwitchDefinition NetworkStatistics::getSwitchDefinition(
        cModule *where) {
    ...
    if (pos == std::string::npos) {
        ...
        sw.isInPort = (std::string::npos
                != where->getFullPath().find("switchPort"));
        cModule *m = where;
        while (nullptr != m) {
